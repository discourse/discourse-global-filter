...
RSpec.describe GlobalFilter::CategoryListSerializerExtension do
  ...
  context "only includes categories allowed per permissions" do
    it "for regular users" do
    ...
    it "for anons" do
      json = CategoryListSerializer.new(CategoryList.new, scope: Guardian.new(), root: false).as_json
      expect(json[:categories].pluck(:id)).to eq([category.id])
    end
  end
end
